...
const SignUpMethod = async(e)=>{
    e.preventDefault();
    try{
        const res = await axios.post(`${import.meta.env.VITE_BASE_URL}auth/master/signUpp`, userData, {
...
return(
    <div>
        <form onSubmit={SignUpMethod} className="flex flex-col space-y-4">
            ...
            <input type="text" name="conformPassword" id="conformPassword" className="p-2 rounded-md bg-[#000000] border border-[#1DCD9F] text-white" required/>
